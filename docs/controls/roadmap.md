...
## 5. Vulnerability management coverage and cadence

**Partially addressed.** Discovery is live on the resolver host and a first triage pass cut
findings by 86% — see [control 07](controls/07-vulnerability-management.md) and the
[kernel false-negative finding](findings/2026-09-07-kernel-remediation-false-negative.md).

**Gap:** one of four endpoints covered, no recurring review interval, and no scanning of the
third-party container images behind every published service. That last one is arguably the
largest, since published applications are the externally reachable surface.

**Plan:**

1. Sample-validate the 79 remaining findings against the upstream security tracker; measure the
   false-positive rate so the count becomes interpretable
2. Remove superseded kernel packages inflating the count
3. Define a recurring patch and review interval — necessarily manual for the container stack,
   where automatic updates are disabled (item 3)
4. Extend agent coverage to the server host
5. Container image scanning

**Done when:** every host with an agent is covered, the residual finding count is validated
rather than assumed, and review happens on a schedule rather than on noticing.

## 6. Service token rotation discipline
...
## 7. Layer-3 network segmentation
